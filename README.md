...
В этом режиме скрываются панели, HUD и логи, остается только итоговый кадр камеры с наложениями/маской. Камера запускается автоматически после загрузки моделей.

В Streamlabs добавьте `Browser Source`, укажите URL выше и размер, например `1280x720` или `1920x1080`. Если Streamlabs не даст браузерному источнику доступ к камере, откройте этот URL в Chrome/Edge, разрешите камеру и добавьте окно браузера через `Window Capture`.

...
Картинки из `public/masks` автоматически попадают в список готовых масок рядом с кнопкой `Изображение`. Пункт `Загруженная` возвращает последнюю маску, выбранную через ручную загрузку файла.
...
Для рук аватара координаты ладони переводятся в экранную систему с учётом режима `Зеркало`, а при доступном Pose Landmarker дополнительно нормализуются относительно плеч/центра тела.
